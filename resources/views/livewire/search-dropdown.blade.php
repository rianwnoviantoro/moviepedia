<div class="relative mt-3 md:mt-0">
  ...
  @if(strlen($search) >= 3)
    <div class="absolute overflow-y-auto bg-gray-800 tex-sm rounded w-64 mt-2 max-h-half">
      @if($searchResults->count() > 0)
        <ul>
          @foreach($searchResults as $result)
            <li class="border-b border-gray-700">
              <a class="block hover:bg-gray-700 transition ease-in-out duration-300 px-2 py-2 flex items-center" href="{{ route('movies.show', $result['id']) }}">
                @if($result['poster_path'])
                  <img class="w-8" src="https://image.tmdb.org/t/p/w92{{ $result['poster_path'] }}" alt="poster_{{ $result['title'] }}">
                @else
